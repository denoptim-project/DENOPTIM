...
|__Interface__||
|`GA-INITMOLSTOFRAGMENTFILE`| Specifies the pathname of a file containing molecules to be considered individuals for the initial population. Each molecule is converted to a candidate by fragmentation and assembling of a DENOPTIM's graph representation. The candidate is then sent to the fitness provider. If the number of molecules is lower than the size ofthe populationDENOPTIM will create additional individuals.|
