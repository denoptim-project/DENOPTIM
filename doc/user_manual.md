...
|`GA-MaxAddFusedRingBridges`| Specifies the maximum number of distinct bridge candidates considered in a single `ADDFUSEDRING` mutation. When more usable bridges are found, a random subset of this size is retained (using the GA random seed) before ring-size biasing. Does not change the relative weight of the `ADDFUSEDRING` mutation type. Default is unlimited (`Integer.MAX_VALUE`).|
|`GA-MaxAddFusedRingSites`| Specifies the maximum number of ring-fusion site combinations considered when performing or locating sites for `ADDFUSEDRING`. When more sites are found, a random subset of this size is retained (using the GA random seed) before ring-size biasing. Default is unlimited (`Integer.MAX_VALUE`).|
|__Interface__||
...
|`FRG-AddExplicitHydrogen`| Requests addition of explicit H atoms on any atom up to satisfaction of the valence. This procedure is sufficiently accurate only for standard organic molecules. Tautomers are not explored.|
|__Fragmentation__||
