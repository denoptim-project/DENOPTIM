...
//TODO: remove obsolete code
            // Define symmetric modification of the graph based on AP symmetry.
            // NB: the combination in the fragsToAdd object already considers 
            // only the first of the symmetryc APs. 
            // See src/DENOPTIM/src/fragspace/FragsCombinationIterator.java 
            Set<SymmetricSet> allNewSymVrtSets = new HashSet<SymmetricSet>();
            if (FragmentSpaceParameters.enforceSymmetry() ||
                FragmentSpaceParameters.symmetryConstraints())
            {
                ArrayList<Integer> vIdsToSkip = new ArrayList<Integer>();
                for (DENOPTIMVertex v : molGraph.getVertexList())
                {
                    if (v.getFreeAPCount()==0 
                        || vIdsToSkip.contains(v.getVertexId()))
                    {
                        continue;
                    }
                    // Define set of vertices that are symmetric to 'v'.
                    // Such set includes 'v' 
                    ArrayList<Integer> symVertices = new ArrayList<Integer>();
                    if (molGraph.hasSymmetricAP())
                    {
                        for (SymmetricSet ss : molGraph.getSymMap())
                        {
                            if (ss.contains(v.getVertexId()))
                            {
                                symVertices = ss.getList();
                                vIdsToSkip.addAll(symVertices);
                                break;
                            }
                        }
                    }
                    ...
                    {
                        symVertices.add(v.getVertexId());
                    }
                    // Deal with APs having sym. APs within the current vertex
                    ArrayList<Integer> doneAps = new ArrayList<Integer>();
                    for (SymmetricSet symSetAps : v.getSymmetricAP())
                    {
                        // ...but only if we imposed symmetry on these APs.
                        // NB: symmetry is not decided here, as it is already
                        // used to select the fragments to add. Still,
                        // we must identify the AP classes for which we are 
                        // imposing a symmetry constrain. If there are 
                        // constraints to the symmetric subtitution ptobability
                        // these are interpreted as
                        // -> "always use symmetry", for p = 100%,
                        // -> or "never use symmetry ", for  p < 100%,
                        // where 'p' is the constrained
                        // probability of symmetric substitution in DEMOPTIM
                        // world)
                        String apClass = v.getAttachmentPoints().get(
                                       symSetAps.getList().get(0)).getAPClass();
                        ...
                        // identify the incoming fragment ID for all symm.APs
                        ArrayList<Integer> newSymVrts =new ArrayList<Integer>();
                        for (Integer apId : symSetAps.getList())
                        {
                            doneAps.add(apId);
                            if(!v.getAttachmentPoints().get(apId).isAvailable())
                            {
                                continue;
                            }
                            // identify which fragToAdd refers to the current 
                            // combination of source vertex and source AP
                            for (IdFragmentAndAP srcFrgAp : fragsToAdd.keySet())
                            {
                                if (v.getVertexId() == srcFrgAp.getVertexId()
                                    && srcFrgAp.getApId() == apId)
                                {
                                    IdFragmentAndAP trgFrgAp = fragsToAdd.get(
                                                                      srcFrgAp);
                                    newSymVrts.add(trgFrgAp.getVertexId());
                                }
                            }
                        }

                        // add the new symm. set of vertices to the symmetry map
                        // of the graph
                        if (newSymVrts.size() > 1)
                        {
                            SymmetricSet newSS = new SymmetricSet(newSymVrts);
                            molGraph.addSymmetricSetOfVertices(newSS);
                        }
                    }

                    // Deal sym. APs on sym. vertices
                    if (symVertices.size() > 1)
                    {
                        // APs with no sym. counterparts within the same vrtx
                        for (int apId=0; apId<v.getNumberOfAP(); apId++)
                        {
                            if(doneAps.contains(apId) ||
                               !v.getAttachmentPoints().get(apId).isAvailable())
                            {
                                continue;
                            }
                            ArrayList<Integer> newSymVrts = 
                                                       new ArrayList<Integer>();
                            for (IdFragmentAndAP srcFrgAp : fragsToAdd.keySet())
                            {
                                if (symVertices.contains(srcFrgAp.getVertexId())
                                    && srcFrgAp.getApId() == apId)
                                {
                                    IdFragmentAndAP trgFrgAp = fragsToAdd.get(
                                                                      srcFrgAp);
