...
echo "Test t1tnk can be run with a default installation of Tinker, ignoring "
...
function compareElementalAnalysis()
{
    ...
    do
        ...
        if [ "$nEl" -ne "$nElRef" ]; then
            echo "Test 't1tnk' NOT PASSED (symptom: wrong number of $el atoms: $nEl, should be $nElRef)"
            exit -1
        fi
    done
}
