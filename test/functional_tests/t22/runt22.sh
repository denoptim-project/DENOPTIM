...
#Check outcome
if ! grep -q 'TestOperator run completed' "$wrkDir"/t22.log
then
    echo " "
    echo "Test 't22' NOT PASSED (symptom: completion msg not found)"
    exit 1
fi

if [ ! -f "graph_mut.sdf" ]; then
    echo "Test 't22' NOT PASSED (symptom: file graph_mut.sdf not found)"
    exit -1
fi
...
if [ "$nChecks" -eq 5 ]
then
    echo "Test 't22' PASSED"
    exit 0
else
    echo "Test 't22' NOT PASSED (sympton: wrong number of checks $nChecks)"
    exit -1
fi
