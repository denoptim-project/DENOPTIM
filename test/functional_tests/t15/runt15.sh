...
exec 2>&1
"$javaDENOPTIM" -jar "$denoptimJar" -r CLG t15-check-1.params
exec 1>&6 6>&-
...
exec 2>&1
"$javaDENOPTIM" -jar "$denoptimJar" -r CLG t15-check-2.params
exec 1>&6 6>&-
...
if ! grep -q 'Completed GeneOpsRunner' "$wrkDir"/t15.log
then
    echo " "
