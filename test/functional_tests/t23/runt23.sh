...
if [ ! -f "graph_mut.sdf" ]; then
    echo "Test 't23' NOT PASSED (symptom: file graph_mut.sdf not found)"
    exit -1
fi
...
do
    ...
    if [ "$nEl" -ne "$nElRef" ]; then
        echo "Test 't23' NOT PASSED (symptom: wrong number of $el atoms in graph_mut.sdf: $nEl, should be $nElRef)"
        exit -1
    fi
    nChecks=$((nChecks+1))
done
...
if [ "$nChecks" -eq 5 ]
then
    echo "Test 't23' PASSED"
    exit 0
else
    echo "Test 't23' NOT PASSED (sympton: wrong number of checks $nChecks)"
    exit -1
fi
