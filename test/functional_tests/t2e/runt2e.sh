...
then
    echo " "
    echo "Test 't2e' NOT PASSED (symptom: wrong number of graphs $nGraphs)"
    exit -1
fi
...
then
    echo " "
    echo "Test 't2e' NOT PASSED (symptom: wrong number of generated rings - $n)"
    exit -1
fi
...
then
    echo " "
    echo "Test 't2e' NOT PASSED (symptom: some fitness value were not reported - found $n)"
    exit 1
fi
...
then
    echo " "
    echo "Test 't2e' NOT PASSED (symptom: completion msg not found)"
    exit -1
else
    echo "Test 't2e' PASSED"
fi
