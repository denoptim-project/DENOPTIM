...
do
    ...
    if ! grep -q 'Completed GraphEditor' "t30-$i.log"
    then
        echo " "
    ...
    if [ ! -f "outGraphs-$i.sdf" ]; then
        echo "Test 't30' NOT PASSED (symptom: file outGraphs-$i.sdf not found)"
        exit -1
    fi

    if [ ! -f "expected_output/outGraphs-$i.sdf" ]; then
        echo "Test 't30' NOT PASSED (symptom: cannot find file with expected results (expected_output/outGraphs-$i.sdf)"
        exit -1
    fi
    ...
    do
        ...
        if [ "$nEl" -ne "$nElRef" ]; then
            echo "Test 't30' NOT PASSED (symptom: wrong number of $el atoms in outGraphs-$i.sdf: $nEl, should be $nElRef)"
            exit -1
        fi
        nChecks=$((nChecks+1))
        #Only for debugging
    ...
    then
        echo "Test 't30' NOT PASSED (sympton: wrong number of checks $nChecks in step $i)"
        exit -1
    fi
    ...
    do
        ...
        if [ "$ng" -ne "$ngRef" ]
        then
            echo "Test 't30' NOT PASSED (sympton: wrong number of $s in step $i)"
            exit -1
        fi
    done
