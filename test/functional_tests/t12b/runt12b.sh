...
then
    echo " "
    echo "Test 't12b' NOT PASSED (symptom: missing UID from initial popl. file)"
    exit 1
fi
...
then
    echo " "
    echo "Test 't12b' NOT PASSED (symptom: missing UID from UIDFileIn)"
    exit 1
fi
...
then
    echo " "
    echo "Test 't12b' NOT PASSED (symptom: completion msg not found)"
    exit 1
else
    echo "Test 't12b' PASSED"
fi
exit 0
