...
then
    echo " "
    echo "Test 't19' NOT PASSED (symptom: output file/s not found)"
    exit 1
fi
...
grep -q 'TestOperator run completed' $wrkDir/t19.log
if [[ $? != 0 ]]
then
    echo " "
    echo "Test 't19' NOT PASSED (symptom: completion msg not found)"
    exit 1
else
