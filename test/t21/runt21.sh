...
if [ ! -f "$wrkDir"/RUN*.log ]
then
    echo "Test 't21' NOT PASSED (symptom: log file not found)"
    exit 1
fi
nUIDs=$(grep "INFO: Selecting graph" "$wrkDir"/RUN*.log | awk '{print $5}' | sort -u | wc -l)
if [ "$nUIDs" -ne 26 ]
then
    echo "Test 't21' NOT PASSED (symptom: wrong number of UIDs)"
    exit 1
fi
...
then
    echo "Test 't21' NOT PASSED (symptom: did not run out of graphs)"
    exit 1
fi
