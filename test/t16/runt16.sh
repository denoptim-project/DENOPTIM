...
while IFS='' read -r uid || [[ -n "$uid" ]]; do
    if ! isInUIDVector "$uid" ; then
        echo " "
        echo "Test 't16' NOT PASSED (symptom: unexpected UID '$uid')"
        exit 1
    fi
...
then
    echo " "
    echo "Test 't16' NOT PASSED (symptom: missing UID from initial popl. file)"
    exit 1
fi
...
then
    echo " "
    echo "Test 't16' NOT PASSED (symptom: missing UID from UIDFileIn)"
    exit 1
fi
