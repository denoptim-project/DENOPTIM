...
echo "RC-CLOSERINGS" >> $DenoptimCGParFile
echo "RC-RCCINDEX=OTF_WDIR/RCCONFS_ID_t4.txt" >> $DenoptimCGParFile
echo "RC-EVALUATIONCLOSABILITYMODE=0" >> $DenoptimCGParFile
echo "RC-CLOSABLERINGSMARTS=C1[C,O]CC[C,O]1" >> $DenoptimCGParFile
echo "RC-CLOSABLERINGSMARTS=C1[C,O]CCC[C,O]1" >> $DenoptimCGParFile
echo "RC-CLOSABLERINGSMARTS=C1[C,O]CCCC[C,O]1" >> $DenoptimCGParFile
echo "RC-VERBOSITY=0" >> $DenoptimCGParFile
...
cp $DenoptimCG3Dout $DenoptimCG3Dout.bkp 
echo "Running UID..."
$java -jar $pathToJarFiles/UpdateUID.jar -m $molUniqueFile -s $DenoptimCG3Dout -k $UIDFILE
echo "After running UID... "
if grep -q "MOL_ERROR" $DenoptimCG3Dout
then
    echo "Found MOL_ERROR"
    cat $DenoptimCG3Dout > $outSDF
    exit $E_OPTERROR
fi
...
nhyd=$($obprop $DenoptimCG3Dout | grep formula | awk -F'[H]' '{print $2}' | awk -F'[A-Z]' '{print $1}')
